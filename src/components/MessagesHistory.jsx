...
const MessagesHistory = ({ mondayUserInstance, messagesHistory, setMessagesHistory, userId }) => {
  const [currentUpdateId, setCurrentupdateId] = useState(null);
  
  const handleDeleteVmFromDb = async (updateId) => {
    const { msg, deletedUpdateId } = await deleteVmFromDb(mondayUserInstance, updateId);
    if (msg === 'success') {
      setMessagesHistory((prev) => {
        let newMessagesHistory = [...prev];
        newMessagesHistory = messagesHistory.filter((message) => message.id !== deletedUpdateId);
  ...
  return (
    <Container>
      {messagesHistory.map(({ id: updateId, assets, body, created_at, creator }, i) => {
        return (
          <Message
            key={i}
